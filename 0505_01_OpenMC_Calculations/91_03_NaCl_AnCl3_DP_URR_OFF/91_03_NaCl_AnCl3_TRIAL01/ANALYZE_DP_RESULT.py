...
POWER_W = 3.35106E+01 # total power (W), constant

# Identify the depleted fuel material in materials.xml
FUEL_MAT_NAME = "NaCl_AnCl3"   # change if your materials.xml uses a different name
...
print("\nEFPD    Burnup(GWd/MTU)   keff        dkeff      beta_eff     dbeta_eff")
print("-------------------------------------------------------------------------")

for t, b, k, ks, be, bes in zip(
    efpd_ifp, burn_ifp, keff_ifp, keff_s_ifp, beta_eff, beta_eff_s
):
    print(f"{t:6.1f}   {b:12.6f}   {k:.6f}  {ks:.6f}  {be:.6e}  {bes:.6e}")